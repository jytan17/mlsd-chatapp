...
**Last completed step:** Phase 3 Step 6 — per-conversation channels w/ dynamic sub/unsub, verified 2-instance (2026-08-03).
**Next step:** open options — (a) `ClientEvent::SendMessage` send over WS, (b) per-user channels as compare/contrast, (c) durability/offline catch-up (read cursors), (d) reliability gaps: subscriber reconnect-on-drop + mid-session conv-join re-subscribe.
**Files in flight:** `Cargo.toml` (single pkg, no workspace), `src/*.rs`, `docker-compose.yml`, `.env`, `Justfile`
