...
**Last completed step:** Phase 3 Step 5 — Redis pub/sub cross-node fanout (global channel), verified 2-instance (2026-08-02).
**Next step:** either (a) migrate global channel → per-conversation channels (dynamic sub/unsub), or (b) `ClientEvent::SendMessage` — send over WS. Also open: subscriber reconnect-on-drop.
