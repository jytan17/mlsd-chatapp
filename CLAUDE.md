...
**Last completed step:** Phase 3 Step 3 — typed WS protocol; server emits `ServerEvent::NewMessage` (2026-08-02).
**Next step:** Phase 3 Step 4 — parse inbound `ClientEvent` in `ws.rs` recv-task (Ping→Pong heartbeat), then Redis pub/sub for cross-node fanout.
