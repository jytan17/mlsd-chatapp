...
**Current phase:** Phase 6 — offline + delivery (read cursor / catch-up)
**Last completed step:** Phase 6 core done — read cursor + mark_read + catch-up `?after=` + UI mark-read button (2026-08-06). Phases 0–4 substantially done; Phase 10 test harness done early.
**Next step:** open options — (a) expose `last_read_message_id` in `GET /conversations` + UI "catch up" button (so unread-only loading is *visible*); (b) unread count badge (`count WHERE id > cursor`); (c) live read receipts (PUBLISH read event on mark_read → other party sees ✓✓); (d) Phase 5 presence/typing; (e) reliability gaps: WS subscriber reconnect-on-drop + mid-session conv-join re-subscribe.
